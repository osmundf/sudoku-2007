...
public class SudokuGUI_Child {
	
	static JFrame frame = new JFrame ("Osmund's Java Sudoku for Childeren.");
	static JMenuBar menuBar = new JMenuBar ();
	
	static JMenu fileMenu = new JMenu ("File");
	static JMenuItem fileMenuOpen = new JMenuItem ("Open");
	static JMenuItem fileMenuSave = new JMenuItem ("Save");
	static JMenuItem fileMenuQuit = new JMenuItem ("Quit");
	
	static JMenu gameMenu = new JMenu ("Game");
	static JMenuItem gameMenuNew = new JMenuItem ("New");
	static JMenuItem gameMenuRestart = new JMenuItem ("Restart");
	static JMenuItem gameMenuSolve = new JMenuItem ("Solve");
	
	static JMenu helpMenu = new JMenu ("Help");
	static JMenuItem helpMenuAbout = new JMenuItem ("About");
	...
	static JPanel puzzlePanel = new JPanel ();
	static JPanel [] numberPanels = new JPanel [9];
	...
	static BufferedImage iconImage = null;
	
	/**
	 * Action listener created specifically for the menu.
	 */
	static ActionListener menuAction = new ActionListener () {
...
//				
			...
			else if (event.getSource () == fileMenuQuit) {
				System.exit (0);
			}
			
			// Game Menu
			else if (event.getSource () == gameMenuNew) {
			...
			else if (event.getSource () == gameMenuRestart) {
				// Setup a default board
				for (int i = 0; i <= 80; i++ ) {
					board [i] = defaultBoard [i];
				}
			...
			else if (event.getSource () == gameMenuSolve) {
				// Clear user input.
				for (int i = 0; i <= 80; i++ ) {
					if (board [i] > 9) {
						board [i] = 0;
	 ...
	 */
	static MouseWheelListener mouseWheeler = new MouseWheelListener () {
		
		public void mouseWheelMoved (MouseWheelEvent e) {
			numberSelection -= e.getWheelRotation ();
			numberSelection = (10 + numberSelection) % 10;
	 ...
	 */
	static MouseListener buttonMouse = new MouseListener () {
	 ...
	 */
	static ActionListener radioAction = new ActionListener () {
...
//			String UI = UIManager.getSystemLookAndFeelClassName ();
			String UI = UIManager.getCrossPlatformLookAndFeelClassName ();
			UIManager.setLookAndFeel (UI);
		}
		catch (Exception e) {
			System.err.println ("Couldn't get specified look and feel (" + "System" + "), for some reason.");
		...
		frame.addMouseWheelListener (mouseWheeler);
		...
		for (int i = 0; i <= 9; i++ ) {
			...
			try {
				radios [i].setIcon (new ImageIcon (SudokuGUI_Child.class.getResource ("/sudoku/images/icon" + i + ".png")));
			}
			catch (Exception e) {
				System.err.println ("Icon could not be loaded for radio button: " + i);
			}
		...
		// Load program icon.
		frame.setIconImage (new ImageIcon (SudokuGUI_Child.class.getResource ("/sudoku/images/icon.png")).getImage ());
		
		// Setup the main frame and display.
		...
		frame.pack ();
		frame.setLocation ((screenBounds.width - frame.getBounds ().width) / 2, (screenBounds.height - frame.getBounds ().height) / 2);
		...
		try {
			...
			for (int i = 0; i < blankTile.length; i++ )
				blankTile [i] = 0xffffffff;
			
			// Puzzle given number icons.
			bufferedImage = ImageIO.read (SudokuGUI_Child.class.getResource ("/sudoku/images/_pc.png"));
			for (int i = 0; i <= 9; i++ )
				buttonIcons [i] = new ImageIcon ((Image) (bufferedImage.getSubimage (i * 40, 0, 40, 40)));
			
			// User placed number icons.
			bufferedImage = ImageIO.read (SudokuGUI_Child.class.getResource ("/sudoku/images/_uc.png"));
			for (int i = 0; i <= 9; i++ )
				buttonIcons [i + 10] = new ImageIcon ((Image) (bufferedImage.getSubimage (i * 40, 0, 40, 40)));
			
			// User index mark icons.
			indexImage = ImageIO.read (SudokuGUI_Child.class.getResource ("/sudoku/images/full_indice.png"));
			for (int i = 0; i <= 511; i++ ) {
				bufferedImage = new BufferedImage (40, 40, BufferedImage.TYPE_INT_ARGB);
				bufferedImage.setRGB (0, 0, 40, 40, indexImage.getRGB (0, 0, 40, 40, new int [40 * 40], 0, 40), 0, 40);
				
				if ((i & 0x001) == 0)
					bufferedImage.setRGB (1, 1, 13, 13, blankTile, 0, 13);
				if ((i & 0x002) == 0)
					bufferedImage.setRGB (14, 1, 13, 13, blankTile, 0, 13);
				if ((i & 0x004) == 0)
					bufferedImage.setRGB (27, 1, 13, 13, blankTile, 0, 13);
				if ((i & 0x008) == 0)
					bufferedImage.setRGB (1, 14, 13, 13, blankTile, 0, 13);
				if ((i & 0x010) == 0)
					bufferedImage.setRGB (14, 14, 13, 13, blankTile, 0, 13);
				if ((i & 0x020) == 0)
					bufferedImage.setRGB (27, 14, 13, 13, blankTile, 0, 13);
				if ((i & 0x040) == 0)
					bufferedImage.setRGB (1, 27, 13, 13, blankTile, 0, 13);
				if ((i & 0x080) == 0)
					bufferedImage.setRGB (14, 27, 13, 13, blankTile, 0, 13);
				if ((i & 0x100) == 0)
					bufferedImage.setRGB (27, 27, 13, 13, blankTile, 0, 13);
